...
describe('DiagramMakerMinimap', () => {
  ...
  const scale = workspaceSize.height / minimapContainerSize.height;
  const minimapWidth = workspaceSize.width / scale;
  const emptyLeft = (minimapContainerSize.width - minimapWidth) / 2;

  const minimapPos = {
    x: minimapContainerPos.x + emptyLeft,
    y: minimapContainerPos.y
  };
  ...
  describe('drag minimap rectangle', () => {
    it('move rectangle and workspace when drag rectangle', () => {
      ...
      const expectedRectTransform = convertTranslate2dToMatrix(
        dragDistance.x + emptyLeft,
        dragDistance.y - borderWidth
      );
  ...
  describe('click minimap canvas', () => {
    it('move rectangle and workspace when click on minimap canvas', () => {
      ...
      const rectOffset = {
        x: clickPos.x - emptyLeft - rectSize.width / 2 + borderWidth,
        y: clickPos.y - rectSize.height / 2 + borderWidth
      };
      ...
      const expectedRectTransform = convertTranslate2dToMatrix(rectOffset.x + emptyLeft, rectOffset.y);
      getMinimapRectangle().should('have.css', 'transform').and('eq', expectedRectTransform);
      const expectedWsTransform = convertTranslate2dToMatrix(workspacePos.x, workspacePos.y);
      getWorkspace().should('have.css', 'transform').and('eq', expectedWsTransform);
    });
