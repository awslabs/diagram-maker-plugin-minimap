...
export function createMinimapRectangularNode(node: DiagramMakerNode<{ odd?: boolean }>, container: HTMLElement) {
  const newDiv = document.createElement('div');
...
export function createMinimapCircularNode(node: DiagramMakerNode<any>, container: HTMLElement) {
  const newDiv = document.createElement('div');
  newDiv.classList.add('circle', 'example-minimap-node');
...
const renderMiniNode = (node: DiagramMakerNode<{}>, container: HTMLElement) => {
  ...
  if (node.typeId === 'testId-input') {
    return createMinimapNodeWithInput(node, container);
  }
  if (node.typeId === 'testId-dropdown') {
    return createMinimapNodeWithDropdown(node, container);
  }
